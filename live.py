...
file_list = eigenface.file_list_from_path("faces/")

O, UT, X = eigenface.train(file_list)
...
while True:
	...
	if len(suspects) > 0:
		...
		for ((x,y,w,h), n) in suspects:
			...
			v = eigenface.vec_from_img(gray_face)
			face_id = eigenface.recognize(O,UT,v,X)

			write(font,img,(5,160),file_list[face_id],(255,255,255))
